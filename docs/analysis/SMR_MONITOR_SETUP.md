# SMR (NuScale Power Corporation) 美股缠论监控配置

**添加日期**: 2026-04-13  
**标的**: SMR (NYSE: NuScale Power Corporation)  
**监控级别**: 日线 + 30 分钟双级别联动
...
# 测试 SMR 数据获取
python -c "import yfinance as yf; t=yf.Ticker('SMR'); print(t.info.get('shortName', 'N/A'))"

# 查看多级别确认状态
python scripts/test_confirmation.py --symbol SMR
```
...
SMR 已加入多级别背驰确认系统 (`scripts/multi_level_confirmation.py`)
...
### 当前状态 (2026-04-13 11:21 EDT)

| 级别 | 背驰状态 | 信号 |
|------|----------|------|
| 日线 (1d) | 🟢 **买背驰** | $11.18 (强度:431.07) |
| 30 分钟 (30m) | ⚪ 无背驰 | 等待确认 |

**联动状态**: ⚠️ **第一阶段已触发** - 等待 30m 确认
...
# SMR 监控已包含在现有配置中 (monitor_all.py)
...
# 3. 测试数据获取
python -c "import yfinance as yf; t=yf.Ticker('SMR'); print(t.history(period='5d'))"
```
...
- [EOSE 监控配置](EOSE_MONITOR_SETUP.md)

---

**配置状态**: ✅ **已启用**  
**多级别联动**: ✅ 日线 +30 分钟联动提醒已配置  
**最后测试**: 2026-04-13 11:20 EDT - 初始配置完成  
