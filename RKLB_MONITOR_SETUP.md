# RKLB (Rocket Lab USA) 美股缠论监控配置

**添加日期**: 2026-04-10  
**标的**: RKLB (NASDAQ: Rocket Lab USA Inc.)  
**监控级别**: 日线 + 30 分钟双级别联动
...
```bash
# 查看警报日志中的 RKLB 信号
grep RKLB alerts.log

# 运行多级别确认分析
python scripts/multi_level_confirmation.py --symbol RKLB
```
...
```bash
# 测试数据获取
python scripts/test_confirmation.py --symbol RKLB
```
...
```bash
# 1. 查看监控日志
tail logs/monitor.log | grep RKLB
...
- [BABA 美股监控配置](BABA_MONITOR_SETUP.md)
...
| BABA | Alibaba Group | 1d + 30m | ✅ 运行中 |
| **RKLB** | **Rocket Lab USA** | **1d + 30m** | ✅ **新增** |
