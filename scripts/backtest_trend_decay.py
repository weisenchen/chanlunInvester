...
"""
趋势衰减监测器 - 回测脚本
Trend Decay Monitor - Backtest Script
...
class TrendDecayBacktester:
    ...
    def __init__(self, symbol_list: List[str], start_date: str, end_date: str):
        ...
        self.monitor = TrendDecayMonitor()
    ...
    def detect_signals(self, series: KlineSeries) -> List[TrendDecaySignal]:
        """检测衰减信号"""
        signals = []
        ...
        for i in range(60, len(series.klines)):
            ...
            signal = self.monitor.monitor(partial_series, series.symbol, '1d')
    ...
    def track_reversal(self, signal: TrendDecaySignal, series: KlineSeries, 
        ...
        result = BacktestResult(
            ...
            decay_probability=signal.decay_probability,
    ...
    def _print_statistics(self, stats: Dict):
        ...
        print(f"预警准确率：  {stats['accuracy']*100:.1f}%")
        ...
        # 验收标准对比
        print("\n验收标准对比:")
        print(f"  预警准确率：  {stats['accuracy']*100:.1f}% / 80% {'✅' if stats['accuracy'] >= 0.80 else '❌'}")
...
def main():
    ...
    # 运行回测
    backtester = TrendDecayBacktester(SYMBOL_LIST, START_DATE, END_DATE)
    ...
    if 'error' in stats:
        ...
        return 1
    elif stats['accuracy'] >= 0.80 and stats['avg_days_to_reversal'] >= 3:
        print("  ✅ Phase 2 回测通过验收标准")
        return 0
    else:
        print("  ❌ Phase 2 回测未通过验收标准")
        return 1
