...
def fetch_uvix_data(timeframe='5m', count=100):
    ...
    if timeframe == '5m':
        ...
        history = uvix.history(period='1mo', interval='1d')
...
def generate_alert(series, results):
    ...
    if results['buy_sell_points']:
        ...
        if 'buy' in bsp['type_en']:
            ...
            print(f"     目标：${bsp['price'] * 1.05:.2f} (+5%)")
        else:
            print(f"  🔴 卖出信号")
            print(f"     入场：${bsp['price']:.2f}")
            print(f"     止损：${bsp['price'] * 1.03:.2f} (+3%)")
            print(f"     目标：${bsp['price'] * 0.95:.2f} (-5%)")
    else:
        # 基于线段方向给出建议
        if results['segments']['latest']:
            ...
            if direction == 'up':
                print(f"  🟢 持有多单")
                print(f"     当前：上涨线段中")
                print(f"     关注：${results['segments']['latest'].start_price:.2f} 支撑")
            else:
                print(f"  🔴 持有空单")
                print(f"     当前：下跌线段中")
                print(f"     关注：${results['segments']['latest'].start_price:.2f} 阻力")
        else:
            print(f"  ⚪ 观望")
            print(f"     等待明确买卖点信号")
