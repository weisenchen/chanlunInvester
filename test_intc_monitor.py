#!/usr/bin/env python3
"""
INTC (Intel Corporation) 美股缠论监控测试脚本
测试 30 分钟和日线级别的数据获取和背驰检测
"""
...
def check_divergence(symbol: str):
    ...
    level30m = analyze_level(symbol, '30m')
    
    if not daily or not level30m:
        print("\n⚠️  无法完成背驰检测（数据不足）")
        return
    
    daily_pens = daily.get('pens', [])
    level30m_pens = level30m.get('pens', [])
    
    print(f"\n{'='*60}")
    print(f"📋 联动分析总结")
    print(f"{'='*60}")
    print(f"日线级别：{len(daily_pens)} 笔，最新价格 ${daily['latest_price']:.2f}")
    print(f"30 分钟级别：{len(level30m_pens)} 笔，最新价格 ${level30m['latest_price']:.2f}")
    
    # 级别联动判断
    if len(daily_pens) >= 2 and len(level30m_pens) >= 2:
        daily_direction = '向上' if daily_pens[-1].is_up else '向下'
        level30m_direction = '向上' if level30m_pens[-1].is_up else '向下'
        
        print(f"\n🔗 级别联动状态:")
        print(f"   日线最新笔：{daily_direction}")
        print(f"   30m 最新笔：{level30m_direction}")
        
        if daily_direction != level30m_direction:
            print(f"\n⚠️  级别不一致 - 可能处于转折期")
        else:
            print(f"\n✅ 级别一致 - 趋势延续中")
    
    print(f"\n✅ INTC 美股缠论分析完成")
    print(f"   监控配置：30m + 1d 双级别联动")
    print(f"   下次自动汇报：16:00 ET (收盘报告)")


if __name__ == '__main__':
    print(f"\n{'='*60}")
    print(f"🇺🇸 INTC (Intel Corporation) 美股缠论监控测试")
    print(f"{'='*60}")
    print(f"时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S EDT')}")
    
    check_divergence('INTC')
