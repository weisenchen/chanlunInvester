#!/usr/bin/env python3
"""
XEG.TO (iShares S&P/TSX Capped Energy Index ETF) 缠论多级别买卖点监控脚本
支持：30 分钟、日线级别
...
def fetch_stock_data(symbol="XEG.TO", period="5d", interval="30m"):
    """获取股票数据"""
    ticker = yf.Ticker(symbol)
    df = ticker.history(period=period, interval=interval)
    return df

def fetch_multi_level_data(symbol="XEG.TO"):
    """获取多级别数据"""
    levels = {
...
def send_alert(message):
    ...
    alert_file = "/home/wei/.openclaw/workspace/chanlun/xeg_alerts.log"
    with open(alert_file, 'a') as f:
        f.write(f"{datetime.now().isoformat()} - {message}\n")

def analyze_and_alert():
    """主分析函数 - 多级别分析"""
    print(f"📊 开始分析 XEG.TO 多级别缠论...")
    
    # 获取多级别数据
    levels_data = fetch_multi_level_data("XEG.TO")
...
⏰ 时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}
    ...
    result_file = "/home/wei/.openclaw/workspace/chanlun/xeg_analysis.json"
